(() => {
    ...
    if (formImage && formPreview && formLabel) {
        ...
        function uploadFile(file) {
            ...
            formLabel.innerHTML = formImage.files[0].name;
    ...
    let formValidate = {
        ...
        formClean(form) {
            ...
            setTimeout((() => {
                ...
                if (formLabel) formLabel.innerHTML = "фотография.jpeg";
    ...
    function initSliders() {
        //! No Autoplay				вимкнути автоматичне прокручування коли користувач не дивиться
        //!===================
        const hero = document.querySelector(".hero");
        const blockHeaders = hero.querySelectorAll(`.block-header`);
        const allAnimEl = hero.querySelectorAll(`h1, h2, h5, .button`);
        function styleElement(transitionProps) {
            if (blockHeaders.length > 0) blockHeaders.forEach((blockHeader => {
                const animEl = blockHeader.querySelectorAll(`h1, h2, h5, .button`);
                animEl.forEach((item => {
                    item.style.transition = transitionProps;
                }));
            }));
        }
        function resetStyle() {
            if (allAnimEl.length > 0) {
                const observer = new MutationObserver(((mutationsList, observer) => {
                    for (let mutation of mutationsList) if (mutation.attributeName === "class") if ([ ...blockHeaders ].some((child => child.classList.contains("_watcher-view")))) {
                        allAnimEl.forEach((item => {
                ...
                observer.observe(hero, config);
            }
        }
        function transition() {
            styleElement([ ".3s, opacity .4s ease .8s", "top .4s ease .8s", "left .4s ease .8s" ]);
        }
        if (document.querySelector(".swiper-hero")) new swiper_core_Swiper(".swiper-hero", {
            ...
            on: {
                ...
                slideChangeTransitionStart: function(swiper) {
                    styleElement([ `${swiper.params.speed}ms` ]);
                },
        ...
        if (document.querySelector(".swiper-delivery")) new swiper_core_Swiper(".swiper-delivery", {
            ...
            speed: 700,
            navigation: {
                prevEl: ".delivery__button-prev",
                nextEl: ".delivery__button-next"
            },
            keyboard: {
                enabled: true,
                onlyInViewport: true,
    ...
    if (document.querySelector(".swiper-advantages")) {
        function checkWindowSize() {
            if (window.innerWidth < 767.98) new swiper_core_Swiper(".swiper-advantages", {
    ...
    da.init();
    //! постійний перезапис змінних сильно навантажує браузер
    window.innerWidth > 767.98 && document.querySelectorAll("[data-list]").forEach((list => {
        list.addEventListener("mouseover", (function(event) {
            const item = event.target.closest("[data-item]");
            if (item && !list.querySelector(".run")) {
                const span = document.createElement("span");
                span.classList.add("run");
                const link = item.querySelector("[href]"), width = item.offsetWidth, height = item.offsetHeight, paddingTop = getComputedStyle(link).paddingTop, paddingTopValue = parseInt(paddingTop);
                span.style.width = width + "px";
                span.style.position = "absolute";
                span.style.height = "6px";
                span.style.left = "0px";
                span.style.bottom = paddingTopValue > 0 ? height - 0 + "px" : height + 2 + "px";
                span.style.opacity = "1";
                span.style.backgroundColor = "rgb(229, 141, 53)";
                span.style.borderRadius = "30% 30% 0 0";
                span.style.zIndex = 100;
                span.style.transition = "all 0.6s ease 0s";
                span.style.pointerEvents = "none";
                span.style.boxShadow = "0 .3125rem .6125rem rgb(229, 141, 53)";
                span.style.filter = "drop-shadow(0 0 5px #E58D35) drop-shadow(0px 10px 10px #E58D35)";
                item.style.position = "relative";
                item.appendChild(span);
                const leftOffset = item.getBoundingClientRect().left + scrollX;
                function handleMouseMove(event) {
                    if (event.target && "mousemove" === event.type && event.target.closest("[data-item]")) {
                        const translateValue = event.target.closest("[data-item]").getBoundingClientRect().left + scrollX - leftOffset;
                        span.style.width = event.target.closest("[data-item]").offsetWidth + "px";
                        span.style.height = "3px";
                        span.style.transform = `translate(${translateValue}px, 0)`;
                    }
                }
                function handleMouseOut(event) {
                    if (event.relatedTarget && "mouseout" === event.type && !event.relatedTarget.closest("[data-list]")) {
    ...
    //! стилі swiper перекривають transition кнопки
        window["FLS"] = false;
    isWebp();
    addTouchClass();
    addLoadedClass();
